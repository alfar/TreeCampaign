...
import { getTerritory, getNeighborhoods, getStreetsByZipCode } from "../../shared/api/client";
...
function houseNumberRange(section: StreetSection): string {
  if (section.startHouseNumber == null && section.endHouseNumber == null) return "alle numre";
...
export default function TerritoryScreen() {
  ...
  const [streetIndex, setStreetIndex] = useState<Map<string, Street>>(new Map());

  useEffect(() => {
    if (!id) return;
    Promise.all([getTerritory(id), getNeighborhoods(id)]).then(
      ([t, hoods]) => {
        setTerritory(t);
  ...
  return (
    <div className="p-4 space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500">Postnr. {territory.defaultZipCode}</p>
      </div>
                  ...
                  .map((section) => (
                    <tr key={section.id} className="border-b last:border-0">
                      <td className="py-1.5 pr-4">
                        {streetIndex.get(section.streetId)?.name ?? <span className="text-gray-400 italic">ukendt</span>}
                      </td>
                      <td className="py-1.5 pr-4 text-gray-600">{houseNumberRange(section)}</td>
                      <td className="py-1.5 pr-4 text-gray-600">{section.sortOrder}</td>
